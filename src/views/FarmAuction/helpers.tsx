...
caduceusTestnetProvider
import { caduceusTestnetProvider } from 'utils/providers'
...
const getDateForBlock = async (currentBlock: number, block: number) => {
  ...
  if (currentBlock > block) {
    try {
      const { timestamp } = await caduceusTestnetProvider.getBlock(block)
...
export const processAuctionData = async (
  ...
  const currentBlock = currentBlockNumber || (await caduceusTestnetProvider.getBlockNumber())
